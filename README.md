...
npx expo start
팁: 웹 브라우저에서 테스트 시 창 너비를 390px~480px로 좁혀 모바일 UI로 확인하시기 바랍니다.
⚙️ 백엔드 (API Server)
Bashcd backend
...
미션 센터 — 생활 루틴이나 업무 인증 미션을 완료하여 포인트를 획득합니다.
갓생점수 확인 — 홈 화면에서 실시간으로 반영되는 갓생점수를 확인합니다.
금융 리포트 — 점수 변화 요인(SHAP 분석)을 확인하고, 하나은행 금리 우대 혜택을 시뮬레이션합니다.
스토어 — 획득한 코인으로 나만의 작업실 아이템을 구매하여 꾸밉니다.


👥 팀 짱사이트 (Team Insight)

구준모 — 팀장 / 기획
권유철 — 개발 (Frontend/Architecture)
조대흠 — 개발 (Backend/ML)
한민영 — 개발 (Design/UI)


하나 청년 금융인재 양성 프로젝트 2026 출품작
text위 Markdown 내용을 그대로 복사하여 GitHub 저장소의 `README.md` 파일에 붙여넣으시면 됩니다. 서식, 테이블, 코드 블록 등이 GitHub에서 최적화되도록 정리하였습니다. 추가 수정이 필요하시면 언제든 말씀해 주십시오.
